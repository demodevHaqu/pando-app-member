🏋️ PANDO Fitness - PHASE 5, 6 완전 구현

🎯 PHASE 5: QR 스캔 기능
Task 5.1: 타입 정의 및 Mock 데이터
파일: types/equipment.ts
typescriptexport interface Equipment {
  id: string;
  name: string;
  nameKo: string;
...
파일: data/mock/equipment.ts
typescriptimport { Equipment } from '@/types/equipment';
...
Task 5.2: QR 스캔 메인 화면
파일: app/qr-scan/page.tsx
...
Task 5.3: 기구 상세 화면
파일: app/qr-scan/equipment/[id]/page.tsx
typescript'use client';
...
Task 5.4: 스트레칭존
파일: app/qr-scan/stretching/page.tsx
typescript'use client';
...
🎯 PHASE 6: 오늘의 운동 루틴
Task 6.1: 타입 정의 및 Mock 데이터
파일: types/routine.ts
typescriptexport interface Routine {
  id: string;
  name: string;
  description: string;
  duration: number;
...
파일: data/mock/routines.ts
typescriptimport { Routine, Exercise } from '@/types/routine';
...
export const MOCK_ROUTINES: Routine[] = [
  ...
  },
  {
    id: 'routine2',
    name: '초급자 전신 운동',
    description: '처음 시작하는 분들을 위한 기초 전신 운동',
    duration: 30,
    exercises: [MOCK_EXERCISES[3], MOCK_EXERCISES[0]],
    difficulty: 1,
    calories: 200,
    type: 'custom',
  ...
  },
  {
    id: 'routine3',
    name: '하체 집중 루틴',
    description: '하체 근력 강화를 위한 집중 루틴',
    duration: 50,
...
export default function RoutineListPage() {
  ...
  const filteredRoutines =
    selectedDuration === 'all'
      ? MOCK_ROUTINES
      : MOCK_ROUTINES.filter((r) => r.duration <= selectedDuration);

  const getDifficultyLabel = (difficulty: number) => {
    const labels = ['', '초급', '중급', '고급', '전문가'];
    return labels[difficulty];
  };

  const getDifficultyColor = (difficulty: number) => {
    if (difficulty <= 2) return 'growth';
    if (difficulty === 3) return 'energy';
    return 'premium';
  };

  return (
    <div className="min-h-screen bg-cyber-dark">
      <Header title="오늘의 루틴" showBack={false} showNotification={true} />

      <div className="p-4 space-y-6">
        {/* 시간 필터 */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <h3 className="text-lg font-bold text-white mb-3">운동 시간</h3>
          <div className="flex gap-2">
        ...
        <motion.div
          ...
          transition={{ delay: 0.1 }}
        >
          <h3 className="text-lg font-bold text-white mb-3">추천 루틴</h3>
          <div className="space-y-4">
            {filteredRoutines.map((routine, idx) => (
              ...
              >
                <Card
                  variant={routine.type === 'ai-generated' ? 'hologram' : 'default'}
                  glow
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {routine.type === 'ai-generated' && (
                          <Badge type="energy" glow>
                            AI
                          </Badge>
                        )}
                      </div>
                      <p className="text-sm text-gray-400 mb-3">{routine.description}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-4">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-gray-300">{routine.duration}분</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Flame size={16} className="text-energy-orange" />
                      <span className="text-gray-300">{routine.calories}kcal</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <TrendingUp size={16} className="text-neon-green" />
                      <Badge type={getDifficultyColor(routine.difficulty) as any}>
                  ...
                  <div className="flex gap-2 mb-4 overflow-x-auto pb-2 hide-scrollbar">
                    {routine.exercises.slice(0, 5).map((exercise, i) => (
                      <div
                        key={i}
                        className="px-3 py-1 glass-dark rounded-full text-xs whitespace-nowrap text-gray-300"
                      >
                        {exercise.nameKo}
                      </div>
                    ))}
                    {routine.exercises.length > 5 && (
                      <div className="px-3 py-1 glass-dark rounded-full text-xs whitespace-nowrap text-gray-500">
                        +{routine.exercises.length - 5}
                      </div>
                    )}
                  </div>
                  ...
                  {/* 시작 버튼 */}
                  <Button
                    variant={routine.type === 'ai-generated' ? 'energy' : 'growth'}
                    size="md"
                    className="w-full"
                    glow={routine.type === 'ai-generated'}
                    shine
                  >
...
Task 6.3: 루틴 상세 화면
파일: app/routine/[routineId]/page.tsx
typescript'use client';
...
import { Play, Clock, Flame, Target, ChevronRight } from 'lucide-react';
import { MOCK_ROUTINES } from '@/data/mock/routines';
...
export default function RoutineDetailPage() {
  ...
  if (!routine) {
    return (
      <div className="min-h-screen bg-cyber-dark flex items-center justify-center">
        <p className="text-white">루틴을 찾을 수 없습니다</p>
      </div>
    );
  }

  const handleStartRoutine = () => {
    router.push(`/routine/${routineId}/exercise/${routine.exercises[0].id}`);
  };

  return (
    <div className="min-h-screen bg-cyber-dark pb-24">
      <Header title={routine.name} showBack={true} showNotification={false} />
      ...
      <div className="p-4 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <Card variant="hologram">
            <div className="flex items-center gap-2 mb-4">
              <h2 className="text-2xl font-bold text-white flex-1">{routine.name}</h2>
              {routine.type === 'ai-generated' && (
                <Badge type="energy" glow>
                  AI 추천
                </Badge>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center p-3 glass-dark rounded-lg">
                ...
                <div className="text-2xl font-bold text-white mb-1">{routine.duration}</div>
                <div className="text-xs text-gray-400">분</div>
              </div>
              <div className="text-center p-3 glass-dark rounded-lg">
                <Flame size={24} className="text-energy-orange mx-auto mb-2" />
                <div className="text-2xl font-bold text-white mb-1">{routine.calories}</div>
                <div className="text-xs text-gray-400">kcal</div>
              </div>
              <div className="text-center p-3 glass-dark rounded-lg">
                <Target size={24} className="text-neon-green mx-auto mb-2" />
                <div className="text-2xl font-bold text-white mb-1">{routine.exercises.length}</div>
                <div className="text-xs text-gray-400">운동</div>
              </div>
            </div>
          </Card>
        </motion.div>
        ...
        <motion.div
          ...
          transition={{ delay: 0.1 }}
        >
          <h3 className="text-lg font-bold text-white mb-3">운동 순서</h3>
          <div className="space-y-3">
            {routine.exercises.map((exercise, idx) => (
              ...
              >
                <Card className="flex items-center gap-4">
                  {/* 순서 번호 */}
                  <div className="w-12 h-12 bg-gradient-energy rounded-full flex items-center justify-center flex-shrink-0">
                  ...
                  {/* 정보 */}
                  <div className="flex-1">
                    <h4 className="font-bold text-white mb-1">{exercise.nameKo}</h4>
        ...
        >
          <Card variant="glass" className="gradient-border">
            <div className="flex items-center gap-3">
              <Flame size={32} className="text-energy-orange" />
              <div className="flex-1">
                <div className="text-sm text-gray-400 mb-1">예상 소모 칼로리</div>
...
Task 6.4: 운동 수행 화면
파일: app/routine/[routineId]/exercise/[exerciseId]/page.tsx
typescript'use client';
...
import ProgressBar from '@/components/ui/ProgressBar';
import Modal from '@/components/ui/Modal';
import { Play, Check, X, Timer, ChevronRight, Trophy } from 'lucide-react';
import { MOCK_ROUTINES } from '@/data/mock/routines';

export default function ExercisePerformPage() {
  const router = useRouter();
  const params = useParams();
  const routineId = params.routineId as string;
  ...
  if (!routine || !exercise) {
    return (
      <div className="min-h-screen bg-cyber-dark flex items-center justify-center">
        <p className="text-white">운동을 찾을 수 없습니다</p>
      </div>
    );
  }
  ...
  const handleNextExercise = () => {
    if (nextExercise) {
      router.push(`/routine/${routineId}/exercise/${nextExercise.id}`);
    } else {
      router.push(`/routine/complete?routineId=${routineId}`);
    }
  };

  const progress = ((exerciseIndex + 1) / routine.exercises.length) * 100;

  return (
    <div className="min-h-screen bg-cyber-dark pb-24">
      <Header
        title={`${exerciseIndex + 1}/${routine.exercises.length}`}
        showBack={true}
      ...
      <div className="p-4 space-y-6">
        {/* 진행률 */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <ProgressBar value={exerciseIndex + 1} max={routine.exercises.length} showLabel color="green" />
        </motion.div>
        ...
        >
          <Card variant="hologram">
            <div className="aspect-video rounded-lg overflow-hidden bg-cyber-mid flex items-center justify-center mb-4">
              {exercise.thumbnailUrl ? (
                <img
                  src={exercise.thumbnailUrl}
                  alt={exercise.nameKo}
                ...
                <Play size={64} className="text-electric-blue" />
              )}
            </div>
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-2xl font-bold text-white">{exercise.nameKo}</h2>
              <Badge type="energy">{exercise.category}</Badge>
            </div>
            <div className="flex flex-wrap gap-2">
              {exercise.muscleGroups.map((muscle, idx) => (
                <span key={idx} className="px-2 py-1 glass-dark rounded text-sm text-gray-300">
                  {muscle}
                </span>
              ))}
            </div>
        ...
        >
          <Card>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-white">세트 진행</h3>
              <span className="text-electric-blue font-bold">
                  ...
                  <button
                    ...
                    className={`py-4 rounded-lg font-bold transition-all ${
                      isCompleted
                        ? 'bg-gradient-growth text-white'
                        : isCurrent
                        ? 'bg-gradient-energy text-white animate-energy-pulse'
                        : 'bg-cyber-mid text-gray-500'
                    }`}
                  >
                    {isCompleted ? <Check size={20} className="mx-auto" /> : setNum}
                  </button>
                );
              })}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-center">
              <div className="p-3 glass-dark rounded-lg">
                <div className="text-sm text-gray-400 mb-1">목표 반복</div>
                <div className="text-2xl font-bold text-white">{exercise.reps}회</div>
              </div>
              {exercise.weight && (
                <div className="p-3 glass-dark rounded-lg">
                  <div className="text-sm text-gray-400 mb-1">무게</div>
                  <div className="text-2xl font-bold text-white">{exercise.weight}kg</div>
                </div>
              )}
            </div>
          </Card>
        ...
        {isResting && (
          ...
          >
            <Card variant="hologram" className="text-center">
              <Timer size={48} className="text-electric-blue mx-auto mb-4 animate-float" />
              <h3 className="text-xl font-bold text-white mb-2">휴식 중</h3>
              <div className="text-6xl font-bold text-gradient-energy mb-4">
        ...
        >
          <Card>
            <h3 className="font-bold text-white mb-3">올바른 자세</h3>
      ...
      >
        <div className="space-y-4">
          <div className="text-center py-4">
            <Trophy size={64} className="text-cyber-yellow mx-auto mb-4 animate-float" />
            <h3 className="text-2xl font-bold text-white mb-2">{exercise.nameKo}</h3>
            <p className="text-gray-400">
              {exercise.sets}세트를 모두 완료했습니다!
            </p>
          </div>
          ...
          {/* 난이도 평가 */}
          <div>
            <div className="text-sm text-gray-400 mb-2">난이도 평가</div>
            <div className="flex gap-2">
              {['너무 쉬움', '적당함', '어려움', '너무 힘듦'].map((label, idx) => (
                <button
                  key={idx}
                  onClick={() => setDifficultyRating(idx + 1)}
...
Task 6.5: 루틴 완료 화면
파일: app/routine/complete/page.tsx
typescript'use client';
...
import Button from '@/components/ui/Button';
import Badge from '@/components/ui/Badge';
import { Trophy, Clock, Flame, Target, TrendingUp, Home, Sparkles } from 'lucide-react';
import { MOCK_ROUTINES } from '@/data/mock/routines';
...
export default function RoutineCompletePage() {
  ...
  return (
    <div className="min-h-screen bg-cyber-dark">
      <Header title="루틴 완료" showBack={false} showNotification={false} />
      ...
      <div className="p-4 space-y-6">
        ...
        >
          <div className="w-40 h-40 bg-gradient-energy rounded-full mx-auto mb-6 flex items-center justify-center animate-scale-pop">
            <Trophy size={100} className="text-white" />
          </div>
          <h1 className="text-4xl font-bold text-gradient-energy mb-2">
            완료했습니다! 🎉
          </h1>
          <p className="text-gray-400 text-lg">정말 멋진 운동이었어요!</p>
        </motion.div>
        ...
        >
          <Card variant="hologram">
            <h3 className="text-xl font-bold text-white mb-4">{routine.name}</h3>
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center p-4 glass-dark rounded-lg">
                <Clock size={32} className="text-electric-blue mx-auto mb-2" />
                <div className="text-2xl font-bold text-white mb-1">{routine.duration}</div>
                <div className="text-xs text-gray-400">분</div>
              </div>
              <div className="text-center p-4 glass-dark rounded-lg">
              ...
              <div className="text-center p-4 glass-dark rounded-lg">
                <Flame size={32} className="text-energy-orange mx-auto mb-2" />
                <div className="text-2xl font-bold text-white mb-1">{routine.calories}</div>
                <div className="text-xs text-gray-400">kcal</div>
              </div>
            </div>
        ...
        >
          <Card className="bg-gradient-growth text-center">
            <TrendingUp size={40} className="text-white mx-auto mb-3" />
            <div className="text-sm text-white/80 mb-2">포인트 획득!</div>
            <div className="text-5xl font-bold text-white mb-2">+{earnedPoints}P</div>
            <p className="text-sm text-white/80">누적 포인트로 혜택을 받아보세요</p>
          </Card>
        </motion.div>
              ...
              {earnedBadges.map((badge, idx) => (
                ...
                >
                  <Card className="text-center">
                    <div className="text-5xl mb-2 animate-scale-pop">{badge.icon}</div>
